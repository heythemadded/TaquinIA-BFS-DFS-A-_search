...
class Taquin:
    ###Constructeur
    def __init__(self,taquin_courant,taquin_precendent,profondeur):
    ...
    for op in operation :
        ...
        if 0<=new_pos.x<3 and 0<=new_pos.y<3 : ##Verifier permuation possible
            ...
            if not (str(nv_taquin) in visited.keys()):
                queue.append(Taquin(nv_taquin,taquin.taquin_courant,taquin.profondeur+1))
...
def main (taquin):
    ...
    queue=[Taquin(taquin,taquin,0)]
    visited={}

    while len(queue)>0:
        ##Soit queue la list des element a traiter.On enleve le dernier 
        # element de la liste queue pour le traiter.Tjr prendre le dernier pour 
        # que les fils decouvert en dernier et le premier a traiter 
        # Rebrousse chepin lorsque le profondeur atteint 3
        ...
        if verif(tq_c.taquin_courant): ##Test-but
            ...
            return len(visited),format(stopt-startt,".8"),chemin(taquin,visited) ##Chemin ,Temps d'exe

        ##Pour chercher le fils de taquin il faut verifier : 
            ## Non vistée 
            # Profondeur <3

        if tq_c.profondeur<65:
            fils(tq_c,queue,visited)
    stopt=timeit.default_timer()
    return "non trouvée","nombre de noeuds visitée="+str(len(visited)),format(stopt-startt,".8")
